...
public class IntegrationTests
{
    ...
    public void testIntegrationWithSphere() {
        ...
        camera.setVPDistance(1).setVPSize(3, 3);
        assertEquals( 18, calcSumIntersection(camera, sphere, 3, 3), "Second test case: should be 18 intersection points");

        // TC03: 10 intersection points
        sphere = new Sphere(new Point(0, 0, -2), 2);
        assertEquals( 10, calcSumIntersection(camera, sphere, 3, 3), "Third test case: should be 10 intersection points");

        // TC04: 9 intersection points
        sphere = new Sphere(new Point(0, 0, -1), 4);
        assertEquals( 9, calcSumIntersection(camera, sphere, 3, 3), "Fourth test case: should be 9 intersection points");

        // TC05: 0 intersection points
        sphere = new Sphere(new Point(0, 0, 1),0.5);
        assertEquals( 0, calcSumIntersection(camera, sphere, 3, 3), "Fifth test case: should be 0 intersection points");
    }
    ...
    public void testIntegrationWithPlane() {
        ...
        assertEquals( 9, calcSumIntersection(camera, plane, 3, 3), "Second test case: should be 9 intersection points");

        // TC03: 6 intersection points
        plane = new Plane(new Point(0, 0, -3), new Vector(0, -1, 1));
        assertEquals( 6, calcSumIntersection(camera, plane, 3, 3), "Third test case: should be 6 intersection points");
    }
    ...
    public void testIntegrationWithTriangle() {
        ...
        assertEquals( 1, calcSumIntersection(camera, tri, 3, 3), "First test case: should be 1 intersection point");

        // TC02: 2 intersection points
        tri = new Triangle(new Point(1, -1, -2), new Point(0, 20, -2), new Point(-1, -1, -2));
        assertEquals( 2, calcSumIntersection(camera, tri, 3, 3), "Second test case: should be 2 intersection points");
    }
    ...
    private int calcSumIntersection(Camera cam, Intersectable body, int nX, int nY) {
        ...
        for (int i = 0; i < nY; ++i) {
            for (int j = 0; j < nX; ++j) {
                var intersections = body.findIntersections(cam.constructRay(nX, nY, j, i));
